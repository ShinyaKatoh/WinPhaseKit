...
int Nst = 130; // maximum number of stations
int Nps = 300000; // maximum number of P/S picks recorded at one station
int Ntb = 8000; // maximum number of lines in traveltime table
...
#pragma omp barrier
    ...
    char ofp1[256], ofp2[256];

    sprintf(ofp1, "%s%s%s", ddir, "/", CATALOGSEL);
    sprintf(ofp2, "%s%s%s", ddir, "/", PHASESEL);
...
#pragma omp barrier
    ...
    char ofp3[256], ofp4[256];

    sprintf(ofp3, "%s%s%s", ddir, "/", HYPOEVENT);
    sprintf(ofp4, "%s%s%s", ddir, "/", HYPOPHASE);
