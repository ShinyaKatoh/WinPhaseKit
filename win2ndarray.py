...
def extract_each_station_waveform_metadata(win_file_path, ch_file_path, target_stations):
    parsed_channel_data, time_index = parse_win_file(win_file_path)
    ch_df = load_ch_table(ch_file_path)
    # print(ch_df.head())
...
#         base_time_np = np.datetime64(f"{year:04d}-{month:02d}-{day:02d}T{hour:02d}:{minute:02d}:{second:02d}")
...
#                 time = base_time_np + np.timedelta64(int(1e6 * i / sampling_rate), 'us')
#                 time_index[ch_id].append(time)
